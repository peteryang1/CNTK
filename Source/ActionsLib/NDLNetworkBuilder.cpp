...
{
	if ((str == L"") && std::is_same<ElemType, TargetType>::value)
		return true;
	if (std::is_same<half, TargetType>::value)
		return EqualCI(str, L"half");
	else if (std::is_same<float, TargetType>::value)
		return EqualCI(str, L"float");
	else if (std::is_same<double, TargetType>::value)
...
void NDLNodeEvaluatorImpl<ElemType>::Evaluate(NDLNode<ElemType>* node, const wstring& baseName, const NDLPass pass)
{
    ...
    if (OperationNameOf(InputValue) == cnNodeType || OperationNameOf(SparseInputValue) == cnNodeType)
    {
        ...
        if (pass == ndlPassInitial)
        {
			...
			else
			{
				...
				else if (EqualCI(precision, L"half"))
				{
					if (isSparse)
				...
				{
					RuntimeError("NDLNetworkBuilder: Input: the 'precision' parameter if specified, must be 'float', 'double' or 'half'");
				}
			}
                //nodePtr = builder.CreateInputNode(name, tensorShape, dynamicAxis);
        }
	...
	else if (cnNodeType == MixedTypedOperationNameOf(CastNode))
	{
		if (parameter.size() < 1)
		...
		if (pass == ndlPassInitial)
		{
			...
			if (EqualCI(targetPrecision, L"half"))
			{
			...
			else if (EqualCI(targetPrecision, L"float"))
			{
				...
				if (isPrecision<ElemType, half>(sourcePrecision))
					...
					RuntimeError("NDLNetworkBuilder: for CastNode to cast to float, input must be 'half' or 'double'");
			}
			else if (EqualCI(targetPrecision, L"double"))
			{
				...
				if (isPrecision<ElemType, half>(sourcePrecision))
					nodePtr = builder2.CreateCastNode<half>(name);
				else if (isPrecision<ElemType, float>(sourcePrecision))
					nodePtr = builder2.CreateCastNode<float>(name);
				else
					RuntimeError("NDLNetworkBuilder: for CastNode to cast to double, input must be 'float' or 'half'");
			}
			else
				RuntimeError("NDLNetworkBuilder: CastNode - need to specify 'precision' parameter: 'float', 'double' or 'half'.");
		}
	}
    else
    {
